...
for (const file of files) {
  test(`${file} contiene los constructores restringidos`, async () => {
    ...
    assert.match(html, /id="page-import-button"/);
    assert.match(html, /Importar página completa/);
    assert.match(html, /id="header-body-files" type="file" multiple/);
    assert.match(html, /id="header-body-list"/);
    assert.match(html, /id="header-bodies-clear"/);
    assert.match(html, /function themeSectionId/);
    assert.match(html, /function themeIndexValue/);
    assert.match(html, /function extractBodyModules/);
    assert.match(html, /function bodyModuleLine/);
    assert.match(html, /function syncHeaderBodies/);
    assert.match(html, /function populateHeaderFromImport/);
    assert.match(html, /Body y cabecera generados/);
    assert.match(html, /sourceRootId/);
